...
public class EditDealerContentPage extends BaseTest {
	public static final Logger logger = LogManager.getLogger(EditDealerContentPage.class);
	SCPages scpages;
	...
	@BeforeClass
	public void dlrEditContentPage_1() {
	...
	@Test(priority = 1)
	public void dlrEditContentPage_2() {
	...
	@Test(priority = 2)
	public void dlrEditContentPage_3() {
			CSVTableRow currentRow = csvData.get("Dealer");
				...
				Assert.assertEquals(actual, true, " Navigate to PageBuilder Page in Deler Level   ");
			}
	
	@Test(priority = 3)
	public void dlrEditContentPage_4() {
	...
	@Test(priority = 4)
	public void dlrEditContentPage_5() {
		CSVTableRow currentRow = csvData.get("Dealer");
				boolean actual = scpages.verifyEditUrl("@Value", currentRow.getString("Url"));
				Assert.assertEquals(actual, true, " Search Content Page Url in Delaer Level ");
			}
	

	@Test(priority = 5)
	public void dlrEditContentPage_6() {
		CSVTableRow currentRow = csvData.get("EditDealer");
	...
	@Test(priority = 6)
	public void dlrEditContentPage_7() {
		CSVTableRow currentRow = csvData.get("EditDealer");
				boolean actual = scpages.selectDepartmentdropdownitem(currentRow.getString("Department"));
				scpages.clickThirdTabNextBtn();
				Assert.assertEquals(actual, true, " Selecting Department in Delaer Level  ");
			}
	

	@Test(priority = 7)
	public void dlrEditContentPage_8() {
		boolean actual = scpages.descriptionLinkButton();
		Assert.assertEquals(actual, true, " Description Button is Not Clickable ");
	}

	@Test(priority = 8)
	public void dlrEditContentPage_9() {
		CSVTableRow currentRow = csvData.get("EditDealer");
				boolean actual = scpages.editResposniveContent(currentRow.getString("Responsive_Content"));
				Assert.assertEquals(actual, true, " Passing Data For Responsive Editor in Delaer Level  ");
			}
	
	@Test(priority = 9)
	public void dlrEditContentPage_10() {
		CSVTableRow currentRow = csvData.get("EditDealer");
				boolean actual = scpages.savePage().veirfyPageTitle(currentRow.getString("PageTitle"));
				Utility.captureScreenshots(driver,"PageBuilder" ,"EditContentPageDealer");
				Assert.assertEquals(actual, true, " Updating Content  Page in Dealer Level ");
			}
