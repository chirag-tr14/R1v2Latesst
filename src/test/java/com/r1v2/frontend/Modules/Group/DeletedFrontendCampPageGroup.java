package com.r1v2.frontend.Modules.Group;
...
public class DeletedFrontendCampPageGroup extends BaseTest {
	public static final Logger logger = LogManager.getLogger(DeletedFrontendCampPageGroup.class);
	DataBase database;
	SCPages pages;
	...
	@Test(priority=1)
	public void deleteFrontendgroupCampaign() throws InterruptedException {

		Iterator<CSVTableRow> itr = page.iterator();
		while (itr.hasNext()) {
			CSVTableRow currentRow = itr.next();
			String level = currentRow.getString("Level");
			if ("FrontendUpdatedGrp".equalsIgnoreCase(level)) {
	...
	@Test(priority = 2,dependsOnMethods={"deleteFrontendgroupCampaign"})
	public void deleteFrontendgroupCampaign_1() {
		CSVTableRow currentRow = csvData.get("FrontendUpdatedGrp");
		String dealerID = currentRow.getString("DealerId");
		String query = "select page_url from izmoweb_r1v2.idw_dealer_webpages where  page_type='CAMP' and"
				+ " fk_dealer_id=" + dealerID + " and user_added=" + userID + " and status='DELE'";
	...
	@Test(priority = 3)
	public void deleteFrontendgroupCampaign_2() {
				boolean actual = pages.verifySiteMapUrl();
				Utility.captureScreenshots(driver,"PageBuilder" ,"FrontendDeleteContentPagegroup");
		Assert.assertEquals(actual, true, " Deleted Campaign Page is not  Loading  FrontEnd in Group Level ");
	}

	@Test(priority = 4)
	public void deleteFrontendgroupCampaign_3() {
